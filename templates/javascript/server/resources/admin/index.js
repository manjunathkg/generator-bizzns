...
function listen (options, callback) {
  ...
  console.log('before connect.basicauth !!!');
  var auth = connect.basicAuth('admin', 'admin');
  console.log('after connect.basicauth !!!');
  ...
  function next(err) {
    ...
    resource.view.create({ path: __dirname + '/view' }, function(err, view) {
      ...
      resource.http.app.get('/admin/resources/:_resource/:_method', auth, function (req, res, next) {
        console.log('Just beore calling present - 1 ***');
        view.method.present({ resource: req.param('_resource'), method: req.param('_method') }, function(err, str){
          console.log('Present - 2 ****** resource: req.param(_resource) ' + req.param('_resource'));
          console.log('Present - 2 ****** resource: req.param(_method) ' + req.param('_method'));
      ...
      resource.http.app.post('/admin/resources/:_resource/:_method', auth, function (req, res, next) {
         ...
         console.log('Just beore calling present - 5 ***  req = ' + util.inspect(req.param('_resource')) );
        var id = req.param('id'),
        ...
        view.method.present({
          ...
          id: id
           ...
           console.log('Inside present - 5 ***  id = ' + id);
          res.end(str);
        });
        console.log('after present - 5 ***  id = ' + id);
